...
def adjust_bag(request, item_id):
    """ Adjust the quantity of the specified products in the bag """
    ...
    if size:
        if quantity > 0:
            ...
            messages.success(
                request,
                f'Removed size {size.upper()} {product.name} to your bag')
    else:
        if quantity > 0:
            bag[item_id] = quantity
            messages.success(
            ...
            bag.pop(item_id)
            messages.success(request, f'Rmoved {product.name} from your bag')
